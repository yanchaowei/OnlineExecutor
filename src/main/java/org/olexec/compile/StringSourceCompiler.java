...
public class StringSourceCompiler {
    ...
    /**
     * 关于Patern的使用:
     * => 根据String类型的正则表达式生成Pattern对象
     * => 根据Pattern的matcher()方法生成Matcher对象
     * => 根据Matcher的相关方法，传入需要匹配的字符串，得到匹配的结果
     */
    /** 使用 Pattern 预编译功能 */
    ...
    public static byte[] compile(String source, DiagnosticCollector<JavaFileObject> compileCollector) {
        ...
        //把源码字符串构造成 重写的JavaFileObject, 供编译使用
        ...
        if (result && bytesJavaFileObject != null) {
            return ((TmpJavaFileObject) bytesJavaFileObject).getCompileByte();
        }
        return null;
    }
    ...
    public static class TmpJavaFileObject extends SimpleJavaFileObject {
         ...
         * 需要传入源码source， 然后调用父类的构造方法创建kind = Kind.SOURCE.extension（.java）的JavaFileObject
         */
         ...
         * 需要传入kind，即我们想要构建一个寻出什么样类型文件的JavaFileObject
         */
